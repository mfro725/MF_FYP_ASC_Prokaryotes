#script to produce distribution of tag rates for 1000 simulated 3'UTR sequence
...
#makes a list of accession folders in UTRs folder
...
cnt = 0 #count for tracking run

#master list of lists to be written into csv file

number_tag_c_randomised_list = [] #list of rows (1000 columns)

column_name_list = [] #list of titles for columns


#inserts column names into master list of lists	
...
column_name_list.insert(0,'observed_tag')
...
list_actual_tag_rates = []

with open('2.1_observed_ASC_rates.csv') as fl_obj:
	reader_obj = csv.reader(fl_obj)
	for row in reader_obj:
		list_actual_tag_rates.append(row[3])
		
del list_actual_tag_rates [0] #removes column title


#loops through files in accession folder to total tag in 3'UTR regions for each randomisation
...
for flder in accession_folder_list:
	...
	for shuffle in range(1000): 
		...
		total_tag_all_files_c = 0
		
		
		for fl_1 in UTR_files:
			
			number_tag_c_shuffle_i = 0
	...
	#tt=11 or tt=4 protein coding totals
			...
			if if_tt_11 == True :
				...
				number_tag_c_shuffle_i = number_tag_c_shuffle_i + numstops_tag
			...
			total_tag_all_files_c = total_tag_all_files_c + number_tag_c_shuffle_i
		
			tot_genes = len(UTR_files)
			tot_coding = tot_genes - tot_non_coding
		
		accession_row_list_c.append(total_tag_all_files_c)
	...
	column_value = cnt - 1
	observed_tag = list_actual_tag_rates[column_value]
	top = float(observed_tag) - (shuffle_mean)
	...
	rows_for_csv = [accession_number, tot_coding, observed_tag, st_dev, shuffle_mean, Z_score, *accession_row_list_c]
		
	rows.append(rows_for_csv) #appends list of lists for csv file

# write into file nul_stop_1

with open('3.1_null1_tag_rates_1.csv', 'w', newline='') as f:
   	writer = csv.writer(f)
