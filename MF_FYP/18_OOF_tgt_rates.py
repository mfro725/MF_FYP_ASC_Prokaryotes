...
#demonstrates dinucleotide preferences
...
UTR_NCDS_path = os.path.join(my_path,'UTR_OOF') 
...
data_columns = ['accession', 'tot_UTRs_OOF', 'tot_seq_OOF', 'num_of_tgt_OOF', 'frequency_tgt_OOF']
...
for flder in accession_folder_list: 
	...
	UTR_files = glob.glob(os.path.join(flder,'*_+2_shifted_UTR.fta')) #list of 3UTR files in accession folder
...
with open('18.1_OOF_tgt_rates.csv', 'w', newline='') as f:
    writer = csv.writer(f)
